...
<html lang="ja">
    ...
    <body>
        <?php
        $cutOffDay = 25;
        $date = mktime(0, 0, 0, 10, 25, 2013);
        $format = 'Y/m/d';
        
        echo '<p>締め日: ' . $cutOffDay . '日</p>';
        echo '<p>計算基準日: ' . date($format, $date) . '</p>';
        
        $ret = getCutOffDate($cutOffDay, $date);

        if ($ret !== FALSE) {
            echo '<p>結果: ' . date($format, $ret) . '</p>';
        } else {
            echo '<p>不正な締め日です</p>';
        }
