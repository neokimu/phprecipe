...
// -----------------------------------------------
 ...
 function dsp_crypt($cfg=0, $reload=1) {
   ...
   // 2009/09/20 Modified by Kenji Suzuki 
   echo "<img id='cryptogram' src='" . $_SESSION['cryptdir'] . 
        "/cryptographp.php?cfg=" . $cfg . "&amp;" . SID . "' alt=\"画像認証\" />";
   if ($reload) {
     echo "<a title='" . ($reload == 1 ? '' : $reload) . 
          "' style=\"cursor:pointer\" onclick=\"javascript:document.images.cryptogram.src='" . 
          $_SESSION['cryptdir'] . "/cryptographp.php?cfg=" . $cfg . "&amp;" . SID . 
          "&amp;'+Math.round(Math.random(0)*1000)+1\"><img src=\"" . 
          $_SESSION['cryptdir'] . "/images/reload.png\" alt=\"再読み込み\" /></a>";
   }
 }
